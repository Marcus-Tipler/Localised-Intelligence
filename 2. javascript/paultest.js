...
function autoFocusInput() {
  const userInput = document.getElementById('#chat-input'); //TODO: USE OF VARIABLE
  //userInput.focus();
}
...
*/
function updateModelInQueryString(model) { //FIXME: FUNCTION CALL
  // make sure browser supports features
  if (window.history.replaceState && 'URLSearchParams' in window) {
    const searchParams = new URLSearchParams(window.location.search); //FIXME: VARIABLE
...
async function populateModels() { //FIXME: FUNCTION CALL
  document.getElementById('nav-icon-download').addEventListener('click', submitRequest); //FIXME: THIS IS THE WRONG BUTTON: THERE IS NO SEND BUTTON.
...
function adjustPadding() {
    console.log("Attempted Padding Adjustments");
...
async function submitRequest() { //TODO: FUNCTION FOR INTERNAL USE (seemingly from this JS file)
  //document.getElementById('.chat-container').style.display = 'block'; //TODO: IT WAS THIS LINE BUT IS NOW ONE BELLOW
  document.getElementById('.chat-container');

  const input = document.getElementById('#chat-input').value; //FIXME: VARIABLE FOR USER INPUT
  const selectedModel = getSelectedModel(); //FIXME: VARIABLE FOR SELECTED MODEL
  const context = document.getElementById('all-chats').context; //FIXME: VARIABLE FOR CONTEXT QND CHAT HISTORY
  const data = { model: selectedModel, prompt: input, context: context }; //FIXME: ALL VARIABLES IN STRING

  // Create user message element and append to chat history
  let chatHistory = document.getElementById('all-chats');
  ...
  //let stopButton = document.createElement('button'); //TODO: CREATION OF A BUTTON
  //stopButton.className = 'btn btn-danger'; //TODO: GIVING BUTTON A CLASS
 // stopButton.innerHTML = 'Stop'; //TODO: GIVING BUTTON SOME TEXT
 // stopButton.onclick = (e) => { //TODO: EVENT MANAGER FOR BUTTON
  //  e.preventDefault();
  //  interrupt.abort('Stop button pressed');
  //}
  // add button after sendButton
  const sendButton = document.getElementById('#nav-icon-download'); //FIXME: VARIABLE SEND BUTTON
  ...
  postRequest(data, interrupt.signal)
    .then(async response => {
      await getResponse(response, parsedResponse => {
        ...
        if (parsedResponse.done) {
          ...
          let copyButton = document.getElementById('#side-bar-btn-duplicate')
    ...
    .then(() => {
        console.log("attempted removal of stop and spinner buttons")
    ...
    .catch(error => {
      ...
      //stopButton.remove();
      //spinner.remove();
      console.log("attempted removal of stop and spinner buttons 2")
    });
...
//    submitRequest(); //FIXME: CALLS INTERNAL JS FUNCTION ABOVE (LINE 152)
//  }
//});
...
window.onload = () => {
  ...
  document.getElementById("nav-icon-delete").addEventListener("click", deleteChat);
...
function deleteChat() {
  //const selectedChat = document.getElementById("chat-select").value; //TODO: WAS THIS LINE IS NOW ONE BELLOW
...
// Function to load selected chat from dropdown FIXME: CAN BE VERY USEFUL HERE:
